...
class COCOKeypointsDataset(ImageDataset):
    ...
    def __getitem__(self, index) -> Tuple[torch.Tensor, IMG_KEYPOINTS_TYPE]:
        ...
        keypoints = self.dataset[index][3]
        if self.transform:
    ...
    def prepare_dataset(self):
        ...
        with open(self.dataset_json_path, "r") as file:
            ...
            annotation_dict = defaultdict(lambda: defaultdict(lambda: []))  # {img_id: {channel : [keypoints for that channel]}}
            ...
            for img_id, keypoint_dict in annotation_dict.items():
                ...
                dataset.append([img_dict[img_id].file_name, img_dict[img_id].height, img_dict[img_id].width,
