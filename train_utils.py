...
from utils.path import get_artifact_dir_path
...
class RelativeEarlyStopping(EarlyStopping):
    ...
    def __init__(self, monitor: Optional[str] = None, min_relative_delta: float = 0.01, patience: int = 3,
                 verbose: bool = False, mode: str = "min", strict: bool = True, ):
    ...
    def _evaluate_stopping_criteria(self, current: torch.Tensor) -> Tuple[bool, Optional[str]]:
        ...
        if self.monitor_op(current * (1 - self.min_delta), self.best_score.to(current.device)):
            ...
            if self.wait_count >= self.patience:
                should_stop = True
                reason = (f"Monitored metric {self.monitor} did not improve in the last {self.wait_count} records."
    ...
    def _improvement_message(self, current: torch.Tensor) -> str:
        ...
        if torch.isfinite(self.best_score):
            msg = (f"Metric {self.monitor} improved {abs(self.best_score - current)/self.best_score:.3f} times >="
...
def create_pl_trainer(hparams: dict) -> Trainer:
    """
