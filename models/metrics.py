...
def keypoint_classification(detected_keypoints: List[DetectedKeypoint], ground_truth_keypoints: List[Keypoint],
                            threshold_distance: int, ) -> List[ClassifiedKeypoint]:
    ...
    ground_truth_keypoints = copy.deepcopy(ground_truth_keypoints)  # make deep copy to do local removals (pass-by-reference..)
    ...
    for detected_keypoint in sorted(detected_keypoints, key=lambda x: x.probability, reverse=True):
        ...
        for gt_keypoint in ground_truth_keypoints:
            ...
            if distance <= threshold_distance + 1e-5:
                classified_keypoint = ClassifiedKeypoint(detected_keypoint.u, detected_keypoint.v,
                                                         detected_keypoint.probability, threshold_distance, True, )
                matched = True
        ...
        if not matched:
            classified_keypoint = ClassifiedKeypoint(detected_keypoint.u, detected_keypoint.v,
                                                     detected_keypoint.probability, threshold_distance, False, )
...
def calculate_precision_recall(classified_keypoints: List[ClassifiedKeypoint],
                               total_ground_truth_keypoints: int ) -> Tuple[List[float], List[float]]:
...
class KeypointAPMetric(Metric):
    ...
    def update(self, detected_keypoints: List[DetectedKeypoint], gt_keypoints: List[Keypoint]):
        ...
        classified_img_keypoints = keypoint_classification(detected_keypoints, gt_keypoints,
                                                           self.keypoint_threshold_distance)
...
if __name__ == "__main__":
    print(check_forward_full_state_property(KeypointAPMetric,
                                            init_args={"keypoint_threshold_distance": 2.0},
                                            input_args={"detected_keypoints": [DetectedKeypoint(10, 20, 0.02)],
