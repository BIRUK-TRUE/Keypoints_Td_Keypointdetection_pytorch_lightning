...
from utils.heatmap import BCE_loss, create_heatmap_batch, get_keypoints_from_heatmap_batch_maxpool
from utils.visualization import (get_logging_label_from_channel_configuration,
                                 visualize_predicted_heatmaps, visualize_predicted_keypoints,)
...
class KeypointDetector(pl.LightningModule):
    ...
    def add_model_argparse_args(parent_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--minimal_keypoint_extraction_pixel_distance", required=False, type=int, default=1,
                            help="the minimal pixel-distance between two keypoints. Allows for some non-maximum"
                                 "surpression.", )
        parser.add_argument("--maximal_gt_keypoint_pixel_distances", required=False, type=str, default="2 4",
        ...
        parser.add_argument("--lr_scheduler_relative_threshold", required=False, default=0.0, type=float,
    ...
    def __init__(self, heatmap_sigma: int, maximal_gt_keypoint_pixel_distances: str,
                 minimal_keypoint_extraction_pixel_distance: int, learning_rate: float, backbone: Backbone,
                 keypoint_channel_configuration: List[List[str]], ap_epoch_start: int, ap_epoch_freq: int,
                 lr_scheduler_relative_threshold: float, max_keypoints: int, **kwargs, ):
        """[summary]
        ...
        head = nn.Conv2d(in_channels=backbone.get_n_channels_out(), out_channels=self.n_heatmaps, kernel_size=(3, 3),
                         padding="same", )
        ...
        self.unnormalized_model = nn.Sequential(backbone, head,)  # NO sigmoid to combine it in loss!(needed for FP16)
    ...
    def shared_step(self, batch, batch_idx, include_visualization_data_in_result_dict=False) -> Dict[str, Any]:
        ...
        gt_heatmaps = [create_heatmap_batch(heatmap_shape, keypoint_channel, self.heatmap_sigma,
                                            self.device) for keypoint_channel in keypoint_channels]
        ...
        for channel_idx in range(len(self.keypoint_channel_configuration)):
            ...
            result_dict.update({f"{self.keypoint_channel_configuration[channel_idx]}_loss":
                                    channel_losses[channel_idx].detach()} )
        ...
        if include_visualization_data_in_result_dict:
            result_dict.update({"input_images": input_images.detach().cpu(),
    ...
    def visualize_predictions_channels(self, result_dict):
        ...
        image_grids = []
        for channel_idx in range(len(self.keypoint_channel_configuration)):
            grid = visualize_predicted_heatmaps(input_images,
    ...
    def visualize_predicted_keypoints(self, result_dict):
        ...
        predicted_keypoints = get_keypoints_from_heatmap_batch_maxpool(predicted_heatmaps, self.max_keypoints,
    ...
    def log_and_reset_mean_ap(self, mode: str):
        ...
        for i, maximal_distance in enumerate(self.maximal_gt_keypoint_pixel_distances):
            self.log(f"{mode}/meanAP/d={float(maximal_distance):.1f}",
    ...
    def update_channel_ap_metrics(self, predicted_heatmaps: torch.Tensor, gt_keypoints: List[torch.Tensor],
    ...
    def compute_and_log_metrics_for_channel(self, metrics: KeypointAPMetrics, channel: str,
                                            training_mode: str ) -> List[float]:
        """
    ...
    def extract_detected_keypoints_from_heatmap(self, heatmap: torch.Tensor) -> List[DetectedKeypoint]:
        ...
        keypoints, scores = get_keypoints_from_heatmap_batch_maxpool(heatmap_to_extract_from, self.max_keypoints,
                                                                     self.minimal_keypoint_pixel_distance,
                                                                     return_scores=True)
        detected_keypoints = [
            ...
            for channel_idx in range(len(detected_keypoints[batch_idx])):
                for kp_idx in range(len(keypoints[batch_idx][channel_idx])):
                    detected_keypoints[batch_idx][channel_idx].append(
                        DetectedKeypoint(keypoints[batch_idx][channel_idx][kp_idx][0],
