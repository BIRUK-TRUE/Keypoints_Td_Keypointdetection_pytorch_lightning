...
class MaxPoolDownSamplingBlock(nn.Module):
    def __init__(self, n_channels_in, n_channels_out, kernel_size):
        ...
        self.conv = nn.Conv2d(in_channels=n_channels_in, out_channels=n_channels_out, kernel_size=kernel_size,
    ...
    def forward(self, x):
        x = self.conv(x)
        x = self.pool(x)
        x = self.relu(x)  # activation and pool are commutative (if activation is monotonic), so pool first to reduce calculations
        x = self.norm(x)  # normalization can be used before or after activation: https://forums.fast.ai/t/order-of-layers-in-model/1261/3
...
class UpSamplingBlock(nn.Module):
    def __init__(self, n_channels_in, n_channels_out, kernel_size):
        super().__init__()
        self.conv = nn.Conv2d(in_channels=n_channels_in * 2, out_channels=n_channels_out, kernel_size=kernel_size,
...
class Unet(Backbone):
    def __init__(self, n_channels_in=3, n_downsampling_layers=2, n_resnet_blocks=3, n_channels=32, kernel_size=3,
                 **kwargs):
        super().__init__()
        ...
        self.downsampling_blocks = nn.ModuleList([MaxPoolDownSamplingBlock(n_channels, n_channels,kernel_size)
                                                  for _ in range(n_downsampling_layers)])
        self.resnet_blocks = nn.ModuleList([ResNetBlock(n_channels, n_channels) for _ in range(n_resnet_blocks)])
        self.upsampling_blocks = nn.ModuleList([UpSamplingBlock(n_channels_in=n_channels, n_channels_out=n_channels,
